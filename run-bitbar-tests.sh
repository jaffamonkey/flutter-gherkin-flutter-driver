...
curl -H 'Content-Type:application/json' -u "$BB_APIKEY": $bitbar_Path/runs --data-binary @configuration-android-"$testrunname"
...
response=$(curl -u "$BB_APIKEY": $bitbar_Path/projects/207073277/runs)
...
while [ $x -le 1 ]; do
  ...
  response2=$(curl -u "$BB_APIKEY": $bitbar_Path/projects/207073277/runs/"$runid")
...
response3=$(curl -u "$BB_APIKEY": $bitbar_Path/projects/207073277/runs/"$runid"/device-sessions)
...
response7=$(curl -u "$BB_APIKEY": $bitbar_Path/projects/207073277/runs/"$runid"/device-sessions/"$devicesessionid1"/output-file-set/files)
...
echo "$testrunname"
curl -L -u "$BB_APIKEY": $filedownload_Path/"$filetodownload1"/file --output console-"$testrunname".log
curl -L -u "$BB_APIKEY": $filedownload_Path/"$filetodownload2"/file --output performance-"$testrunname".json
curl -L -u "$BB_APIKEY": $filedownload_Path/"$filetodownload3"/file --output output-files-"$testrunname".zip
curl -L -u "$BB_APIKEY": $filedownload_Path/"$filetodownload4"/file --output device-"$testrunname".log
ls -la
